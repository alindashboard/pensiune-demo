...
export default async function ContactPage({ searchParams }: PageProps) {
  ...
  const showResolved = tab === 'rezolvate'

  const supabase = createSupabaseAdminClient()

  const [{ data: unresolved }, { data: resolved }] = await Promise.all([
    supabase.from('contact_requests').select('*').eq('resolved', false).order('created_at', { ascending: false }),
    supabase.from('contact_requests').select('*').eq('resolved', true).order('created_at', { ascending: false }),
  ])

  const unresolvedList: ContactRequest[] = unresolved ?? []
  const resolvedList: ContactRequest[] = resolved ?? []
  const displayed = showResolved ? resolvedList : unresolvedList
    ...
    <AdminShell activeSection="contact">
      <div className="p-6">
        ...
        <div className="flex gap-2 mb-6 border-b pb-0">
          {[
            { label: 'Nerezolvate', value: '', count: unresolvedList.length },
            { label: 'Rezolvate', value: 'rezolvate', count: resolvedList.length },
          ].map((t) => (
            <Link
              key={t.value}
              href={t.value ? `/admin/contact?tab=${t.value}` : '/admin/contact'}
              className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px ${
                showResolved === (t.value === 'rezolvate')
        ...
        {displayed.length === 0 ? (
          <div className="text-center py-20 text-muted-foreground">
            Nu există formulare {showResolved ? 'rezolvate' : 'nerezolvate'}.
          </div>
        ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex gap-2 shrink-0">
                      {!req.resolved && (
                        <form action={resolve}>
                          <Button type="submit" size="sm" variant="default">Rezolvat</Button>
                        </form>
                      )}
                      <ConfirmFormButton
                        action={remove}
                        message={`Ștergi cererea de la ${req.name}? Acțiune ireversibilă.`}
                      >
                        Șterge
                      </ConfirmFormButton>
                    </div>
